...
def lstm_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        if pm.DECAY_LEARNING_RATE_ACTIVE:
            ...
            decayed_learning_rate = learning_func
...
def bidir_lstm_model_fn(features, labels, mode, params):
    ...
    loss += 0.001*regularization_cost
    #loss += 0.00001*regularization_cost
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        #learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(10*one_epoch_in_step, tf.int64)), lambda: 0.1*learning_rate, lambda: 0.01*learning_rate)
        if str(pm.AMBIGUITY) == 'è_e' or str(pm.AMBIGUITY) == 'e_i':
            ...
            # è_e pos
            #learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(3.0*one_epoch_in_step, tf.int64)), lambda: learning_rate / 5.0, lambda: learning_rate / 10.0)
            #learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(300, tf.int64)), lambda: learning_rate/10.0, lambda: learning_rate/1000.0)
            #learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(50, tf.int64)), lambda: learning_rate, lambda: learning_func)
        # For ho_o, ha_a
        elif str(pm.AMBIGUITY) == 'ha_a' or str(pm.AMBIGUITY) == 'ho_o' or str(pm.AMBIGUITY) == 'hai_ai':
            learning_func = learning_rate
            ...
            learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(25*one_epoch_in_step, tf.int64)), lambda: learning_rate, lambda: learning_rate/10.0)
            #learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(15.0*one_epoch_in_step, tf.int64)), lambda: learning_rate / 5.0, lambda: learning_rate / 70.0)
            #learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(5*one_epoch_in_step, tf.int64)), lambda: learning_rate, lambda: learning_func)
        elif str(pm.AMBIGUITY) == 'hai_ai':
            learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(40.0*one_epoch_in_step, tf.int64)), lambda: learning_rate / 5.0, lambda: learning_rate / 50.0)
            learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(25*one_epoch_in_step, tf.int64)), lambda: learning_rate, lambda: learning_func)
        # For sara_sarà
        elif str(pm.AMBIGUITY) == 'sarà_sara':
        ...
        elif str(pm.AMBIGUITY) == 'hanno_anno':
            learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(50*one_epoch_in_step, tf.int64)), lambda: learning_rate/50.0, lambda: learning_rate/100.0)
            learning_func = tf.cond(tf.less(tf.train.get_global_step(), tf.cast(25*one_epoch_in_step, tf.int64)), lambda: learning_rate, lambda: learning_func)
        elif str(pm.AMBIGUITY) == 'dovè_dove':
...
def cnn_model_fn(features, labels, mode, params):
    ...
    training = (mode == tf.estimator.ModeKeys.TRAIN)
    dropout_emb = tf.layers.dropout(inputs=word_embeddings, rate=0.2, training=training)
    # convolution: a sentence can be seen like an image with dimension length x 1 (that's why conv1d)
    ...
    # words_conv_1 = tf.layers.conv1d(words_conv_1, filters=filters, kernel_size=4,
    #                               strides=stride, padding='valid', activation=tf.nn.relu)
    # words_conv_2 = tf.layers.conv1d(words_conv_2, filters=filters, kernel_size=3,
    #                               strides=stride, padding='valid', activation=tf.nn.relu)
    # words_conv_3 = tf.layers.conv1d(words_conv_3, filters=filters, kernel_size=2,
    #                               strides=stride, padding='valid', activation=tf.nn.relu)
    ...
    if pm.PRINT_SHAPE:
        ...
        print('----------------------------->  concat:', words_conv.get_shape())
